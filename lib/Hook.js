...
Hook.prototype.listen = function( options, cb ) {
	...
	if ( cb == null && options && options instanceof Function ) cb = options;
	cb = cb || function( ) {};
	var self = this;
	var server = self._server = nssocket.createServer( function( socket ) {
		...
		socket.data( 'sHook::hello', function( d ) {
			client.name = d.name;
		} );
		...
		socket.data( 'sHook::off', function( d ) {
		...
		socket.data( 'sHook::emit', function( d ) {
...
Hook.prototype.connect = function( options, cb ) {
	...
	cb( );
	var client = this._client = new nssocket.NsSocket( {
	...
	client.data( 'sHook::pushemit', function( d ) {
		EventEmitter.prototype.emit.apply( self, [ d.event, d.data ] );
	} );
